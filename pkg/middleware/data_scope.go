...
// ApplyDataScope 应用数据权限（用于查询时自动过滤数据）
// 参数：
//   - db: GORM数据库对象
//   - user: 当前用户信息
//   - tableName: 表名（如 "sys_user"）
//   - deptIDColumn: 部门ID字段名（如 "dept_id"）
//   - userIDColumn: 用户ID字段名（如 "create_by" 或 "id"）
//
// 用法：
...
// GetUserDataScope 获取用户数据权限信息（供业务层使用）
